...
public partial class Main : Node2D
{
	...
	public override void _Process(double delta)
	{
		...
		// Update camera
		camera.GlobalPosition = new Vector2(camera.GlobalPosition.X, Math.Min(player.GlobalPosition.Y, lastValidY));

		// Generate new platforms when player goes higher
		...
		// Update UI
		ui.UpdateUI(player, platforms.Count, powerups.Count(p => !p.IsCollected));
	}
	...
	private void ShowGameOver()
	{
		...
		var scoreLabel = gameOverScreen.GetNode<Label>("ScoreLabel");
		scoreLabel.Text = $"Final Score: {player.Score:F0}";
	}
	...
	private void ResetGame()
	{
		...
		foreach (Node child in platformsContainer.GetChildren())
		{
			child.QueueFree();
		}
		foreach (Node child in powerupsContainer.GetChildren())
		{
			child.QueueFree();
		}
	...
	private void GeneratePlatforms()
	{
		while (platforms.Count < maxPlatforms)
		{
			...
			while (tooFar && attempts < 10)
			{
				...
				tooFar = platforms.Any(p => Math.Abs(p.GlobalPosition.X - newX) > 300);
				attempts++;
			}
	...
	private void CleanupPlatforms()
	{
		...
		var platformsToRemove = platforms.Where(platform => platform.GlobalPosition.Y > cleanupThreshold).ToList();
	...
	private void CleanupPowerups()
	{
		...
		var powerupsToRemove = powerups.Where(powerup => powerup.GlobalPosition.Y > cleanupThreshold || powerup.IsCollected).ToList();
